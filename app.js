...
function showDetail(hero) {
  ...
  const strengthsHtml = (hero.strengths || []).map(s => `<span class="power-chip" style="border-color:#22c55e; color:#86efac;">${s}</span>`).join('');
  const weaknessesHtml = (hero.weaknesses || []).map(w => `<span class="power-chip" style="border-color:#ef4444; color:#fca5a5;">${w}</span>`).join('');
  ...
  panel.innerHTML = `
    ...
    <div class="detail-body">
      ...
      <div class="stats-section">
        ...
        <div class="powers-list">${hero.powers.map(p => `<span class="power-chip">${p}</span>`).join('')}</div>
      </div>
      ${hero.strengths ? `<div class="stats-section"><h4 style="color:#86efac;">Strengths</h4><div class="powers-list">${strengthsHtml}</div></div>` : ''}
      ${hero.weaknesses ? `<div class="stats-section"><h4 style="color:#fca5a5;">Weaknesses</h4><div class="powers-list">${weaknessesHtml}</div></div>` : ''}
