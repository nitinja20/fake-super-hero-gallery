...
function renderHeroes(list = heroes) {
  ...
  if (list.length === 0) {
    grid.innerHTML = `<p style="grid-column:1/-1; text-align:center; color:#94a3b8; padding:40px;">No heroes found matching your filters.</p>`;
    return;
  }
...
function showDetail(hero) {
  ...
  panel.innerHTML = `
    ...
    <div class="detail-body">
      <div class="detail-name">${hero.name} <span style="font-size:0.9rem; color:#a855f7; font-weight:500;">${hero.title}</span></div>
      ...
      <div class="stats-section">
        <h4>Power Stats <span style="color:#94a3b8; font-weight:400; font-size:0.85rem;">(Avg ${avg})</span></h4>
        <div class="stat-bars">
...
document.getElementById('saveTeamBtn').addEventListener('click', () => {
  ...
  alert(`Team saved!\n\n${names.join(' • ')}\n\n(This is a demo – team is stored only for this session)`);
});
